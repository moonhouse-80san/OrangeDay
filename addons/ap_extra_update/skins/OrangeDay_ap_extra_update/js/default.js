jQuery(document).ready(function($) {
	...
	$(document).on('click', '.ap_extra_update_command .btn-c', function() {
		if ( $(this).index() === 1 ) {
...
function updateExtraInfo(document_srl, rel, values) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			if ( data.old_category_srl && typeof data.category_srl !== 'undefined' && (data.old_category_srl !== data.category_srl) ) {
				location.reload();
